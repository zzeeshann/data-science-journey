...
A desktop application that downloads and runs open-weight language models locally and exposes them through an OpenAI-compatible HTTP API on `localhost:1234`. The app handles model download, [quantization](#quantization-4-bit) selection, GPU/CPU memory allocation, and the OpenAI-API contract, presenting a single Start/Stop server button. Session 14 used LM Studio as the Phase-A serving stack with [Qwen 2.5 14B Instruct](https://huggingface.co/Qwen/Qwen2.5-14B-Instruct) at MLX 4-bit quantization. The stack stood up and ran the AG News validation, but engine bugs surfaced under further use; the user uninstalled LM Studio and moved to [Ollama](#ollama) for Phase B. The entry stays in the glossary because LM Studio's design (OpenAI-compatible localhost server, GUI for model management) is the prototype the rest of the local-LLM tooling now imitates, and the Ollama swap was *one line* in `ask_qwen()` precisely because both speak the same API. **Currently not in use in this project.** First used in [session_14.md](sessions/session_14.md).

## Local LLM

A language model running on the user's own machine rather than as a cloud API call. The category is defined by *where the inference happens*, not by what the model can do — a "local LLM" can be a 1-billion-parameter laptop model or a 70-billion-parameter workstation model. The trade-off vs. a cloud model: smaller maximum size and slower for very large models, but private (no data leaves the machine), free per token (no API bill), available offline, and reproducible (the exact model weights stay frozen on disk and don't shift under your feet between runs). This project's first local LLM is Qwen 2.5 14B Instruct, served by [Ollama](#ollama) on the user's M4 Pro Mac and added as the project's second analytical primitive in [Session 14](sessions/session_14.md) alongside the [embedding](#embedding) pipeline from Sessions 7–9. The two primitives are complementary: embeddings answer *what is this close to in semantic space*, a local LLM with a prompt answers *what is this saying, in our terms*. First used in [session_14.md](sessions/session_14.md).

## Ollama

A terminal-based local LLM runtime. Installed with a single `curl | sh` command, then drives entirely through a small `ollama` CLI — `ollama pull qwen2.5:14b` downloads the model and stores it in `~/.ollama/`, `ollama serve` runs the inference server (auto-started on macOS login), `ollama list` shows what's available. The server speaks the OpenAI chat-completions contract on `localhost:11434`, so any Python client that talks to OpenAI's cloud can talk to a local Ollama instance by changing the URL and the model name. Ollama defaults to **GGUF Q4_K_M** [quantization](#quantization-4-bit), which is the canonical 4-bit format outside of Apple's MLX ecosystem and what `ollama pull qwen2.5:14b` actually fetches (~9 GB). Session 14 switched to Ollama as the Phase-B serving stack after Phase A's [LM Studio](#lm-studio) hit engine bugs; Ollama is the working stack at end of session. The case for Ollama over LM Studio in the second DeepSeek tutorial: terminal-stable, no model-format confusion (GGUF/MLX/Safetensors), auto-runs in background. The case for LM Studio: GUI for browsing and configuring models. For this project's purposes (one model, scripted use), Ollama's stability advantage decides it. First used in [session_14.md](sessions/session_14.md).
...
A compression technique that shrinks a language model's file size and memory footprint by storing each of its numerical [parameters](#parameters) with fewer bits of precision than it was originally trained at. A full-precision parameter takes 16 or 32 bits; a 4-bit-quantized parameter takes 4 bits — roughly a 4–8× reduction in disk and RAM. The trade-off is small accuracy loss: at 4-bit, modern quantization schemes typically lose 1–3% of benchmark performance, which is often invisible at the application level. Both 4-bit variants of Qwen 2.5 14B that Session 14 touched land at ~9 GB instead of the ~28 GB the full-precision weights would need, which is what lets a 14-billion-parameter model run comfortably on a 24 GB M4 Pro Mac:

- **MLX 4-bit** is Apple's Mac-optimised numerical library's quantization format — the build the project's Phase-A [LM Studio](#lm-studio) instance served.
- **GGUF Q4_K_M** is the canonical 4-bit format outside the MLX ecosystem and the default that `ollama pull qwen2.5:14b` fetches — the build the project's Phase-B [Ollama](#ollama) instance serves. Numerically the two are not bit-identical at inference (different rounding choices in different libraries) but both deliver the same usable-quality model at the same file-size budget. First used in [session_14.md](sessions/session_14.md).
