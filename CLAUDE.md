...
**State:** The Wholeness Investigation closed at Session 9. Sessions 10–13 wrapped the book. The book is now a complete, conclusive artefact — ten chapters, thirteen session records, glossary, plan, audit notes, and four summary files at repo root ([`mistakes_made.md`](mistakes_made.md), [`summary_for_a_reader.md`](summary_for_a_reader.md), [`improvements.md`](improvements.md), [`reader_glossary_audit.md`](reader_glossary_audit.md)). The project itself stays open. Session 14 (2026-05-11) opened the post-book project: a **local LLM analysis stack** now runs on the user's Mac Mini M4 Pro (24 GB RAM, macOS 26.3) — **Ollama** serving `qwen2.5:14b` (GGUF Q4_K_M, ~9 GB) on `localhost:11434`, driven from **JupyterLab 4.5.7** via the OpenAI-compatible chat-completions API, at ~28 tokens/sec. The session ran in three iterations on the same day: Phase A on LM Studio + Qwen MLX 4-bit hit specific engine errors (`libpython3.11.dylib` not found, then a safetensors-format mismatch); Phase A.5 tried the `jupyter-ai` v3.0.0 extension and abandoned it for a plain `ask_qwen()` function; Phase B switched to Ollama and is the working stack. The consolidated setup guide is at [`data/raw/deepseek_local_llm_guide_2026_05_11_ollama_consolidated.html`](data/raw/deepseek_local_llm_guide_2026_05_11_ollama_consolidated.html). Capability acquisition, not a finding. The next investigation question has not been chosen.
...
**Last session committed:** [Session 14](sessions/session_14.md) — local LLM stack stood up on the user's Mac. Three iterations on the same day: Phase A on LM Studio + Qwen MLX 4-bit (specific engine errors documented in receipts: `libpython3.11.dylib` not found, then safetensors-format mismatch), Phase A.5 tried `jupyter-ai` v3.0.0 extension and abandoned it for UI bugs, Phase B on **Ollama** serving `qwen2.5:14b` + plain `ask_qwen()` function + **JupyterLab 4.5.7** (working stack at ~28 tok/sec). AG News 50-article validation at 92% accuracy reported in all three tutorials; whether Phase B was independently re-run is undetermined from receipts. The consolidated setup guide ([`data/raw/deepseek_local_llm_guide_2026_05_11_ollama_consolidated.html`](data/raw/deepseek_local_llm_guide_2026_05_11_ollama_consolidated.html)) is the canonical commands reference for rebuilding this stack. Project's analytical toolkit now has two primitives — embeddings (Sessions 7–9, runs on Colab+T4) and LLM-classification-via-prompt (Session 14, runs on user's Mac via Ollama).
...
data/raw/deepseek_local_llm_guide_2026_05_11_ollama.html
data/raw/deepseek_local_llm_guide_2026_05_11_ollama_consolidated.html
