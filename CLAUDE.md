...
**Last session committed:** [Session 14](sessions/session_14.md) — local LLM stack stood up on the user's Mac. Two phases on the same day: Phase A on LM Studio + Qwen MLX 4-bit (engine bugs surfaced, abandoned), Phase B on **Ollama** serving `qwen2.5:14b` (working stack). AG News 50-article validation at 92% accuracy reported in both tutorials as a tutorial-grade sanity check; whether the Phase-B figure was independently re-run is undetermined from receipts. Project's analytical toolkit now has two primitives — embeddings (Sessions 7–9, runs on Colab+T4) and LLM-classification-via-prompt (Session 14, runs on user's Mac via Ollama).
